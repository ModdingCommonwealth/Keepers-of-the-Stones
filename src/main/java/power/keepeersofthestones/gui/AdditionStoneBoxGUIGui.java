...
public class AdditionStoneBoxGUIGui extends PowerModElements.ModElement {
	public static HashMap guistate = new HashMap();
	private static ContainerType<GuiContainerMod> containerType = null;

	public AdditionStoneBoxGUIGui(PowerModElements instance) {
		super(instance, 93);
	...
	private static class ContainerRegisterHandler {
		...
		public void registerContainer(RegistryEvent.Register<ContainerType<?>> event) {
			event.getRegistry().register(containerType.setRegistryName("addition_stone_box_gui"));
		}
	}

	@OnlyIn(Dist.CLIENT)
	public void initElements() {
		DeferredWorkQueue.runLater(() -> ScreenManager.registerFactory(containerType, AdditionStoneBoxGUIGuiWindow::new));
	}
	...
	public static class GuiContainerMod extends Container implements Supplier<Map<Integer, Slot>> {
		...
		public GuiContainerMod(int id, PlayerInventory inv, PacketBuffer extraData) {
			...
			this.customSlots.put(1, this.addSlot(new SlotItemHandler(internal, 1, 61, 26) {
				...
				public boolean isItemValid(ItemStack stack) {
					return (IceStoneItem.block == stack.getItem());
				}
			}));
			this.customSlots.put(2, this.addSlot(new SlotItemHandler(internal, 2, 97, 26) {
				@Override
				public boolean isItemValid(ItemStack stack) {
					return (LightningStoneItem.block == stack.getItem());
				}
			}));
			this.customSlots.put(3, this.addSlot(new SlotItemHandler(internal, 3, 61, 62) {
				@Override
				public boolean isItemValid(ItemStack stack) {
					return (SoundStoneItem.block == stack.getItem());
				}
			}));
			this.customSlots.put(4, this.addSlot(new SlotItemHandler(internal, 4, 97, 62) {
				@Override
				public boolean isItemValid(ItemStack stack) {
					return (CrystalStoneItem.block == stack.getItem());
				}
			}));
			this.customSlots.put(5, this.addSlot(new SlotItemHandler(internal, 5, 43, 44) {
				@Override
				public boolean isItemValid(ItemStack stack) {
					return (LavaStoneItem.block == stack.getItem());
				}
			}));
			this.customSlots.put(6, this.addSlot(new SlotItemHandler(internal, 6, 115, 44) {
				@Override
				public boolean isItemValid(ItemStack stack) {
					return (RainStoneItem.block == stack.getItem());
				}
			}));
			this.customSlots.put(7, this.addSlot(new SlotItemHandler(internal, 7, 79, 8) {
				@Override
				public boolean isItemValid(ItemStack stack) {
					return (TornadoStoneItem.block == stack.getItem());
				}
			}));
			this.customSlots.put(8, this.addSlot(new SlotItemHandler(internal, 8, 79, 44) {
				@Override
				public boolean isItemValid(ItemStack stack) {
					return (OceanStoneItem.block == stack.getItem());
				}
			}));
			int si;
			int sj;
		...
		private void slotChanged(int slotid, int ctype, int meta) {
			if (this.world != null && this.world.isRemote()) {
				PowerMod.PACKET_HANDLER.sendToServer(new GUISlotChangedMessage(slotid, x, y, z, ctype, meta));
