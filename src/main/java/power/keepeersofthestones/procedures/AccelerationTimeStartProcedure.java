	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		if (dependencies.get("world") == null) {
			if (!dependencies.containsKey("world"))
				PowerMod.LOGGER.warn("Failed to load dependency world for procedure AccelerationTimeStart!");
			return;
		}
		...
		if (world instanceof ServerWorld && ((World) world).getServer() != null) {
			((World) world).getServer().getCommandManager().handleCommand(
					...
					String.format("gamerule %s %d", (GameRules.RANDOM_TICK_SPEED).toString(), (int) 100));
		}
	}
}
