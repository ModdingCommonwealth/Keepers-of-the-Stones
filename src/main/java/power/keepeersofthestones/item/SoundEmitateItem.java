...
import power.keepeersofthestones.gui.SoundEmitateChoiceGuiWindow;
...
public class SoundEmitateItem extends PowerModElements.ModElement {
	...
	public void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == block) {
			if (Minecraft.getInstance().currentScreen instanceof SoundEmitateChoiceGuiWindow) {
			...
			if (entity instanceof ServerPlayerEntity) {
				NetworkHooks.openGui((ServerPlayerEntity) entity, new INamedContainerProvider() {
					...
					public ITextComponent getDisplayName() {
						return new StringTextComponent("Sound Emitate");
					}
					...
					public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
						...
						return new SoundEmitateChoiceGui.GuiContainerMod(id, inventory, packetBuffer);
					}
				}, buf -> {
